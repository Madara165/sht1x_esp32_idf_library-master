...
float readTempF(int data, int clk) {
  ...
  const float D1 = -39.4; // at 3.3V
  const float D2 = 0.018; // for 14 bit data

  obtVal = readTemperatureRaw(data, clk); // get the reading from sensor (raw // data)
...
esp_err_t sht1x_init(gpio_num_t data_pin, gpio_num_t clc_pin) {
  ...
  for (;;) {
    ...
    printf("-> Humidity: %.2f%% \n", humidity-12.5f /*+ 125 * ((float)humidity / max_adc) + 93)*/);
    printf("\n");
